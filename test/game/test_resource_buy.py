...
class ResourcesBuyTestCase(BaseTest):
    def setUp(self):
    ...
    def _send_resource_buy(self, resources):
    ...
    def test_not_your_turn(self):
        ...
        with self.user_logged_in(self.user_1.id):
            received = self._send_resource_buy({
                'coal': 20,
                'oil': 17,
                'uranium': 10,
                'waste': 22,
    ...
    def test_too_many_resources_requested(self):
        with self.user_logged_in(self.user_1.id):
            received = self._send_resource_buy({
                'coal': 0,
                'oil': 11,
    ...
    def test_game_does_not_have_so_much_resources(self):
    ...
    def test_not_enough_cash(self):
        Player.cash.write(redis, 44, self.user_1.id)
        with self.user_logged_in(self.user_1.id):
            received = self._send_resource_buy({
                'coal': 0,
                'oil': 10,
    ...
    def test_next_user(self):
        ...
        self.assertEqual(game.order, [
            self.user_2.id, self.user_3.id, self.user_1.id
        ])

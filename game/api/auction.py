...
def auction_bid_transaction(pipe, user_id, bid, station):
    ...
    folded_user_ids = game.auction_user_ids.union(game.auction_passed_user_ids)
    next_user_id = game.get_next_user_id(user_id, exclude_ids=folded_user_ids)
...
def auction_pass_transaction(pipe, user_id):
    ...
    if game.has_active_station_in_auction():
        ...
        if len(users_left) == 1:
            ...
            Game.turn.write(pipe, next_user_id, game_id)
            pipe.sadd(Game.auction_passed_user_ids.key(game_id), user_id)
    else:
        if game.phase == 0:
            raise EnergynetException('You cannot pass')
        else:
            _remove_stations_on_pass(game, map_config)

            if game.get_users_left_for_auction() == {player.id}:
                _move_to_next_stage(pipe, game)

                if game.phase == 0:
                    Game.phase.write(pipe, 1, game.id)
            else:
...
def _remove_stations_on_pass(game, map_config):
    pass
...
def _game_station_bought_next_turn_user(pipe, game, player):
    next_user_id = game.get_next_user_id(
        player.id, exclude_ids=game.auction_user_ids
    )
    if next_user_id:
